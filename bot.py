...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = """
🔐 *Универсальный бот с функцией шифрования*
...
# Команда /chats (список всех чатов, где есть бот)
async def list_chats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показывает список всех групп, где есть бот (только для владельца)"""
    if not is_owner(update):
        await update.message.reply_text(" ")
        return

    try:
        # Получаем обновления, чтобы найти чаты
        updates = await context.bot.get_updates()
        chats = {}
        
        # Собираем информацию о чатах из апдейтов
        for upd in updates:
        ...
        text = "📋 *Список групп, где есть бот:*\n\n"
        for chat_id, info in chats.items():
            text += f"*{info['title']}*\n"
            text += f"🆔 ID: `{chat_id}`\n"
            text += f"📌 Тип: {info['type']}\n"
            if info['username']:
                text += f"🔗 Юзернейм: @{info['username']}\n"
            text += "───────────────\n"
...
# Команда /chatinfo (подробная информация о чате)
async def chat_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показывает подробную информацию о чате или указанной группе"""
    if not is_owner(update):
        await update.message.reply_text(" ")
        return
    ...
    # Если есть аргументы, пытаемся получить информацию по ID или юзернейму
    if context.args:
        try:
            ...
            await update.message.reply_text("❌ Не удалось найти чат. Проверь ID или юзернейм.")
            return
    else:
        # Текущий чат
        chat = update.effective_chat
        chat_id = chat.id
        chat_title = chat.title or "Текущий чат"
    ...
    try:
        ...
        text = f"🔍 *Подробная информация о чате:*\n\n"
        text += f"*Название:* {chat.title or 'Без названия'}\n"
        text += f"*ID:* `{chat.id}`\n"
        text += f"*Тип:* {chat.type}\n"
        ...
        if chat.username:
            text += f"*Юзернейм:* @{chat.username}\n"
        if chat.description:
            text += f"*Описание:* {chat.description[:200]}\n"
        if chat.invite_link:
            text += f"*Ссылка:* {chat.invite_link}\n"
        
        # Информация о правах бота
        bot_member = await context.bot.get_chat_member(chat_id, context.bot.id)
        text += f"*Права бота:* {bot_member.status}\n"
...
# Команда /say (написать сообщение от имени бота в указанную группу)
async def say_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отправляет сообщение от имени бота в указанную группу"""
    if not is_owner(update):
        await update.message.reply_text(" ")
        return
    ...
    if len(context.args) < 2:
        await update.message.reply_text(
            "❌ *Неверный формат!*\n\n"
            "Использование:\n"
            "`/say [чат_id или @чат] [текст сообщения]`\n\n"
            "*Примеры:*\n"
            "`/say -100123456789 Всем привет!`\n"
            "`/say @channelname Важное объявление!`",
            parse_mode='Markdown'
        )
        return

    # Получаем идентификатор чата (ID или юзернейм)
    chat_identifier = context.args[0]
    ...
    try:
        ...
        if chat_identifier.startswith('@'):
            ...
            chat_name = chat.title or f"чат {target_chat_id}"
        ...
        # Логируем действие
        logger.info(f"Сообщение отправлено в {chat_name} ({target_chat_id})")
        ...
        await update.message.reply_text(
            ...
            f"`{e}`\n\n"
            f"Проверь:\n"
            f"1. Правильный ли ID/юзернейм\n"
            f"2. Есть ли бот в этой группе\n"
            f"3. Есть ли у бота права писать",
            parse_mode='Markdown'
        )

# Команда /broadcast (рассылка во все группы)
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            "❌ *Укажи текст для рассылки!*\n\n"
            "Пример: `/broadcast Важное объявление для всех групп!`",
            parse_mode='Markdown'
        )
        return
    ...
    try:
        ...
        for upd in updates:
            if upd.message and upd.message.chat:
                chat = upd.message.chat
                if chat.type in ['group', 'supergroup'] and chat.id not in chats:
                    chats[chat.id] = {
        ...
        if not chats:
            await status_msg.edit_text("❌ Бот не состоит ни в одной группе.")
            return
        ...
        for chat_id, chat_info in chats.items():
            try:
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"📢 *Рассылка от владельца:*\n\n{message_text}",
                    parse_mode='Markdown'
                )
                success += 1
                logger.info(f"Рассылка успешно отправлена в {chat_info['title']}")
            except Exception as e:
                failed += 1
                failed_list.append(f"{chat_info['title']}: {str(e)[:50]}")
                logger.error(f"Ошибка рассылки в {chat_info['title']}: {e}")

        # Отчет о рассылке
        report = f"📊 *Отчет о рассылке*\n\n"
        report += f"✅ Успешно: {success}\n"
        report += f"❌ Неудачно: {failed}\n"
        report += f"📋 Всего групп: {len(chats)}\n\n"

        if failed_list:
            report += "*Ошибки:*\n"
            for fail in failed_list[:5]:  # Показываем первые 5 ошибок
                report += f"• {fail}\n"
        ...
        await status_msg.edit_text(f"❌ Ошибка при рассылке: {e}")

# Команда /leave (выйти из указанной группы)
async def leave_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Бот покидает указанную группу"""
    if not is_owner(update):
        await update.message.reply_text(" ")
        return
    ...
    if not context.args:
        await update.message.reply_text(
            "❌ *Укажи ID группы!*\n\n"
            "Использование: `/leave [чат_id]`\n"
            "Чтобы узнать ID групп, используй /chats",
            parse_mode='Markdown'
        )
        return
    ...
    try:
        ...
        confirm_msg = await update.message.reply_text(
            f"⚠️ *Точно выйти из группы* `{chat_name}`?\n"
            f"Отправь `ДА` для подтверждения или отмена через 10 секунд",
            parse_mode='Markdown'
        )
        ...
        try:
            ...
            response = await context.bot.wait_for(
                'message',
                timeout=10,
                check=lambda m: m.from_user.id == YOUR_USER_ID and m.chat.id == update.effective_chat.id
            )
            
            if response.text.upper() == 'ДА':
                await context.bot.leave_chat(chat_id)
                await update.message.reply_text(f"✅ Бот покинул группу: {chat_name}")
                logger.info(f"Бот покинул группу {chat_name} ({chat_id})")
            else:
                await update.message.reply_text("❌ Выход отменен.")
                
        except asyncio.TimeoutError:
            await update.message.reply_text("⏱ Время ожидания истекло. Выход отменен.")

    except Exception as e:
        logger.error(f"Ошибка при выходе из группы: {e}")
        await update.message.reply_text(f"❌ Ошибка: {e}")

# Команда /forward (переслать сообщение в другую группу)
async def forward_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Пересылает сообщение из текущего чата в указанную группу"""
    if not is_owner(update):
        await update.message.reply_text(" ")
        return
    ...
    if not update.message.reply_to_message:
        await update.message.reply_text(
            ...
            "Использование:\n"
            "1. Ответь на сообщение командой `/forward [чат_id или @чат]`",
            parse_mode='Markdown'
        )
        return
    ...
    if not context.args:
        await update.message.reply_text(
            "❌ *Укажи, куда переслать!*\n\n"
            "Пример: `/forward -100123456789` или `/forward @channelname`",
            parse_mode='Markdown'
        )
        return
    ...
    try:
        ...
        if chat_identifier.startswith('@'):
            target_chat = await context.bot.get_chat(chat_identifier)
            target_chat_id = target_chat.id
            chat_name = target_chat.title or chat_identifier
        else:
            target_chat_id = int(chat_identifier)
            target_chat = await context.bot.get_chat(target_chat_id)
            chat_name = target_chat.title or f"чат {target_chat_id}"
        ...
        await update.message.reply_text(f"✅ Сообщение переслано в: {chat_name}")

    except Exception as e:
        logger.error(f"Ошибка при пересылке: {e}")
        await update.message.reply_text(f"❌ Ошибка: {e}")

# Команда /copy (скопировать сообщение в другую группу без ссылки)
async def copy_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Копирует сообщение в другую группу (без указания автора)"""
    if not is_owner(update):
        await update.message.reply_text(" ")
        return
    ...
    if not update.message.reply_to_message:
        await update.message.reply_text(
            "❌ *Ответь на сообщение, которое хочешь скопировать!*\n\n"
            "Использование:\n"
            "1. Ответь на сообщение командой `/copy [чат_id или @чат]`",
            parse_mode='Markdown'
        )
        return
    ...
    if not context.args:
        await update.message.reply_text(
            "❌ *Укажи, куда скопировать!*\n\n"
            "Пример: `/copy -100123456789` или `/copy @channelname`",
            parse_mode='Markdown'
        )
        return
    ...
    try:
        ...
        # Копируем сообщение (отправляем как новое от бота)
        ...
        await update.message.reply_text(f"✅ Сообщение скопировано в: {chat_name}")
...
def main():
    ...
    app.add_handler(CommandHandler("copy", copy_message))
